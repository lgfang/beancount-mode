...
Returns 'year, 'month, 'day, or nil if not on a date."
  (when (thing-at-point-looking-at beancount-date-regexp 10)
    (let* ((date-start (match-beginning 0))
           ...
           (separator (if (string-match-p "/" date-string) "/" "-")))
      (cond
       ;; Year: positions 0-3
...
If cursor is on year, increment year; on month, increment month; on day, increment day."
  (interactive "p")
...
If cursor is on year, decrement year; on month, decrement month; on day, decrement day."
  (interactive "p")
  (beancount--shift-date-component-at-point (- (or n 1))))
